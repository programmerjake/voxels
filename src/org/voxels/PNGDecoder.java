...
public class PNGDecoder
{
    ...
    public boolean hasAlphaChannel()
    {
        return this.colorType == COLOR_TRUEALPHA
                || this.colorType == COLOR_GREYALPHA;
    }
    ...
    {
        return hasAlphaChannel() || this.paletteA != null
                || this.transPixel != null;
    }
    ...
    public void decode(final ByteBuffer buffer,
                       final int stride,
                       final Format fmt) throws IOException
    {
        final int offset = buffer.position();
        final int lineSize = ((this.width * this.bitdepth + 7) / 8)
                * this.bytesPerPixel;
     ...
     * @throws UnsupportedOperationException
     *             if the image can't be decoded into the desired format */
    public void decodeFlipped(final ByteBuffer buffer,
                              final int stride,
                              final Format fmt) throws IOException
    {
        if(stride <= 0)
    ...
    private void
        unfilter(final byte[] curLine, final byte[] prevLine) throws IOException
    {
        switch(curLine[0])
        {
        case 0: // none
    ...
    private int readChunk(final byte[] buffer,
                          final int offset,
                          final int length_in) throws IOException
    {
    ...
    private void readChunkUnzip(final Inflater inflater,
                                final byte[] buffer,
                                final int offset_in,
                                final int length_in) throws IOException
    {
        int offset = offset_in, length = length_in;
    ...
    private void readFully(final byte[] buffer,
                           final int offset_in,
                           final int length_in) throws IOException
    {
        int offset = offset_in, length = length_in;
    ...
    private void skip(final long amount_in) throws IOException
    {
        long amount = amount_in;
        while(amount > 0)
        {
