...
public class Block implements GameObject
{
	                             ...
	                             boolean isItemGlowing)
	{
		...
		glColor4f(c1, c1, c1, 1.0f);
		glTexCoord2f(u1, v1);
		glVertex(p1);
		glColor4f(c2, c2, c2, 1.0f);
		glTexCoord2f(u2, v2);
		glVertex(p2);
		glColor4f(c3, c3, c3, 1.0f);
		glTexCoord2f(u3, v3);
		glVertex(p3);
		glColor4f(c4, c4, c4, 1.0f);
		glTexCoord2f(u4, v4);
		glVertex(p4);
		if(doublesided)
			drawFace(p1,
	...
	private void internalDraw(int drawMask,
	                          Matrix localToBlock,
	                          Matrix blockToWorld,
	                          int bx,
	                          int by,
	...
	private void drawFluidFace(Vector nunv,
	                           Vector punv,
	                           Vector pupv,
	                           Vector nupv,
	                           float minu,
	...
	private void drawFluid(Matrix blockToWorld,
	                       int bx,
	                       int by,
	                       int bz,
	                       Image img)
	...
	// private void
	private void drawSolid(Matrix blockToWorld,
	                       int bx,
	                       int by,
	                       int bz,
	...
	private void drawSim3D(Matrix localToBlock,
	                       Matrix blockToWorld,
	                       int bx,
	                       int by,
	                       int bz,
	...
	@Override
	public void draw(Matrix blockToWorld)
	{
		draw(blockToWorld, false, false);
	}
	...
	{
		final float xzscale = 0.1f, yscale = 0.7f;
		Matrix tform = Matrix.translate(-0.5f, 0.0f, -0.5f)
	...
	private void draw(Matrix blockToWorld, boolean isEntity, boolean isAsItem)
	{
		int bx, by, bz;
		...
		case BDTCustom:
			...
			{
				Image ImgBase = this.type.textures[(this.data.orientation & 0xA)
				...
				drawItem(Matrix.rotatex(Math.PI / 2).concat(tform),
				         blockToWorld,
				         bx,
				         by,
				         bz,
			...
			case BTLeaves:
				...
				break;
			case BTLadder:
				drawItem(Matrix.translate(-0.5f, -0.5f, -0.49f)
		...
		case BDTItem:
			...
			break;
		case BDTSolid:
			drawSolid(blockToWorld, bx, by, bz, false, isEntity, isAsItem);
			break;
		case BDTNone:
			break;
		case BDTTorch:
		...
		case BDTLiquid:
		{
			...
			{
				drawFluid(blockToWorld, bx, by, bz, this.type.textures[0]);
			}
			break;
		}
		case BDTButton:
		...
		case BDTSolidAllSides:
			drawSolid(blockToWorld, bx, by, bz, true, isEntity, isAsItem);
			break;
		default:
			break;
		}
	...
	private boolean rayIntersectsBlock(Vector origin,
	                                   Vector direction,
	                                   Matrix tform)
	{
		Matrix invtform = tform.invert();
		Vector orig = invtform.apply(origin);
		Vector dir = invtform.applyToNormal(direction);
	 ...
	 * @return true if the ray intersects this block
	 */
	public boolean rayIntersects(Vector dir,
	                             Vector invdir,
	                             Vector pos,
	                             Vector hitpos)
	{
	 ...
	 *            image to draw
	 */
	public static void drawImgAsEntity(Matrix blockToWorld, Image img)
	{
	 ...
	 *            matrix to transform block coordinates to world coordinates
	 */
	public void drawAsEntity(Matrix blockToWorld)
	{
		switch(this.type)
		{
		case BTDeleteBlock:
		...
		case BTRedstoneTorchOn:
		{
			...
			b.draw(Matrix.scale(1.42857f).concat(blockToWorld), true, false);
			return;
		}
		case BTStoneButton:
		case BTWoodButton:
	 ...
	 *            coordinates
	 */
	public void drawAsItem(Matrix blockToWorld)
	{
		switch(this.type)
		{
		case BTDeleteBlock:
