...
public abstract class MenuScreen {
	...
	protected static abstract class TextMenuItem extends MenuItem {
		...
		public TextMenuItem(final String text, final Color textColor,
				final Color backgroundColor, final Color selectedTextColor,
				final Color selectedBackgroundColor, final MenuScreen container) {
			super(
					Math.max(Text.sizeH("A"), Text.sizeH(text)) * 2.0f * 2.0f / 480.0f,
					container);
			this.text = text;
			this.textColor = new Color(textColor);
			this.backgroundColor = new Color(backgroundColor);
		...
		protected void drawBackground(final Matrix tform,
			...
			rs.setMatrix(tform);
			rs.concatMatrix(Matrix.setToScale(drawBackground_t1, 1.0f,
					getHeight(), 1.0f));
			rs.setMatrix(rs.getMatrix().concat(drawBackground_t1,
					Matrix.setToScale(drawBackground_t2, 1.1f)));
			rs.addRect(-1.0f, 0, 1.0f, 1.0f, 0, 0, 1, 1, backgroundColor,
					texture);
			rs.render();
		...
		public void draw(final Matrix tform, final boolean isSelected) {
			...
			Matrix mat = Matrix
					.setToScale(draw_t1, this.textScale, this.textScale, 1.0f)
					.concatAndSet(
							Matrix.setToTranslate(draw_t2, -this.textScale / 2f
									* Text.sizeW(this.text) / Text.sizeW("A"),
									this.textScale * Text.sizeH(this.text)
											/ Text.sizeH("A") - this.textScale,
									0)).concatAndSet(tform);
	...
	protected static abstract class CheckMenuItem extends TextMenuItem {
		...
		public CheckMenuItem(final String text, final Color textColor,
				final Color backgroundColor, final Color selectedTextColor,
				final Color selectedBackgroundColor, final MenuScreen container) {
			super(text, textColor, backgroundColor, selectedTextColor,
					selectedBackgroundColor, container);
		}
		...
		public CheckMenuItem(final String text, final Color textColor,
				...
				final TextureAtlas.TextureHandle selectedTexture,
				final MenuScreen container) {
			super(text, textColor, texture, selectedTextColor, selectedTexture,
					container);
		}
	...
	protected static class SpacerMenuItem extends MenuItem {
		...
		public void draw(final Matrix tform, final boolean isSelected) {
			...
			Main.opengl.glColor4f(Color.GetRValue(this.color) / 255.0f,
					...
					Color.GetBValue(this.color) / 255.0f, 0.0f);
			Main.opengl.glBegin(Main.opengl.GL_LINES());
			Vector.glVertex(tform.apply(draw_t1,
					Vector.set(draw_t1, -1.0f, 0.5f * getHeight(), 0)));
			Vector.glVertex(tform.apply(draw_t1,
					Vector.set(draw_t1, 1.0f, 0.5f * getHeight(), 0)));
			Main.opengl.glEnd();
		}
	...
	protected static abstract class ProgressMenuItem extends MenuItem {
		...
		public void draw(final Matrix tform, final boolean isSelected) {
			...
			Vector.glVertex(tform.apply(draw_t1,
					Vector.set(draw_t1, rightside, 0.5f * getHeight(), 0)));
			Vector.glVertex(tform.apply(draw_t1,
					Vector.set(draw_t1, rightside, 0.5f * getHeight(), 0)));
			Vector.glVertex(tform.apply(draw_t1,
					Vector.set(draw_t1, -1.0f, 0.5f * getHeight(), 0)));
	...
	protected static abstract class OptionMenuItem extends TextMenuItem {
		...
		public OptionMenuItem(final String text, final Color textColor,
				final Color backgroundColor, final Color selectedTextColor,
				final Color selectedBackgroundColor, final MenuScreen container) {
			super(text, textColor, backgroundColor, selectedTextColor,
					selectedBackgroundColor, container);
